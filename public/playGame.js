...
class playGame extends Phaser.Scene {
    ...
    create(data) {
        ...
        this.socket.on('currentPlayersGame', function (players) {
            console.log("All players");
            console.log(players);
        });

        this.player1ScoreText = this.add.text(16, 16, '', { fontSize: '32px', fill: '#0000FF' });
        this.player2ScoreText = this.add.text(584, 16, '', { fontSize: '32px', fill: '#FF0000' });

        this.socket.on('scoreUpdate', function (players) {
            console.log(players);
            self.player1ScoreText.setText("");
            let newText = "";
            ...
            self.player1ScoreText.setText(newText);
        });
    ...
    drawCircle(i) {
        ...
        this.circles[i].x = this.config.width / 2 + this.randomOption(gameOptions.distanceFromCenter) * Phaser.Math.RND.sign();
        ...
        if (i == 0 && this.bottomCircle == 0) {
            ...
            this.circles[i].y = config.height - radius - gameOptions.bottomDistance;
        }
        else {
        ...
        if (this.canShoot) {
            ...
            this.ballTween = this.tweens.add({
                ...
                onComplete: function () {
                    ...
                    this.socket.emit('levelPassed');

                    // determine the amount of pixels to scroll to make top circle move down to the bottom of the canvas
                    let yScroll = config.height - this.circles[targetIndex].radius - gameOptions.bottomDistance - this.circles[targetIndex].y
